...
// use 24 bits for the Accumulators. 
   ...
   fir256d #("coefEa.mif", ABITS, TAPS) A (reset, clock, waddr, raddr, weA, x_real, x_imag, RaccAa, IaccAa);            // first bank odd coeff
   fir256d #("coefEb.mif", ABITS, TAPS) B (reset, clock, (waddr + 7'd64), raddr, weA, x_real, x_imag, RaccAb, IaccAb);  // second bank 
   fir256d #("coefFa.mif", ABITS, TAPS) C (reset, clock, waddr, raddr, weB, x_real, x_imag, RaccBa, IaccBa);            // first bank even coeff
   fir256d #("coefFb.mif", ABITS, TAPS) D (reset, clock, (waddr + 7'd64), raddr, weB, x_real, x_imag, RaccBb, IaccBb);  // second bank  
...
module fir256d(
   ...
   firromH #(MifFile) roma (caddr, clock, coef);      // coefficient ROM 18 X 128
